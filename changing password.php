...
if (strlen($_SESSION['user_id']) == 0) {
    ...
    if(isset($_POST['update_password'])) {
        ...
        $verify_sql = "SELECT ID FROM users WHERE id = :user_id AND password = :current_password";
        $verify_query = $dbh->prepare($verify_sql);
        $verify_query->bindParam(':user_id', $user_id, PDO::PARAM_INT);
        $verify_query->bindParam(':current_password', $current_password, PDO::PARAM_STR);
...
<html lang="en">
<head>
    <title>User Management System - Change Password</title>
    <script>
