...
if(isset($_POST['submit']))
{
    ...
    $requester_name = $_POST['full_name'];
    $requester_email = $_POST['email_address'];
    $requester_phone = $_POST['phone_number'];
    $blood_need = $_POST['blood_need_reason'];
    $requester_message = $_POST['request_message'];

    // Prepare SQL statement for insertion
    $insert_query = "INSERT INTO tbl_blood_requests (DonorID, RequesterName, RequesterEmail, RequesterPhone, BloodNeedReason, RequestMessage) 
                     VALUES (:donor_id, :requester_name, :requester_email, :requester_phone, :blood_need, :requester_message)";

    // Prepare and execute the query
    $stmt = $dbh->prepare($insert_query);
    $stmt->bindParam(':donor_id', $donor_id, PDO::PARAM_STR);
    $stmt->bindParam(':requester_name', $requester_name, PDO::PARAM_STR);
    $stmt->bindParam(':requester_email', $requester_email, PDO::PARAM_STR);
    $stmt->bindParam(':requester_phone', $requester_phone, PDO::PARAM_STR);
    $stmt->bindParam(':blood_need', $blood_need, PDO::PARAM_STR);
    $stmt->bindParam(':requester_message', $requester_message, PDO::PARAM_STR);
    $stmt->execute();
    ...
    if($inserted_id)
    {
        echo '<script>alert("Your blood request has been submitted. We will contact you soon.");</script>';
    }
    else 
    {
        echo "<script>alert('An error occurred. Please try submitting your request again.');</script>";  
    }
}
?>
session_start();
